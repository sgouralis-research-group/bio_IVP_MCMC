...
for rep = 1:params.T_rep
    ...
    % prepare sampler M=4
    sample_T = params.T_sig .* randn(params.M,1);
    tempor_T = params.T_sig .* randn(params.M,1);
...
h = gamrnd((params.N * params.K) /2 + params.h_prior_phi ...
    ,get_B(y, x(:,2),params) );
...
out = 0; 
inner = 0;
x = get_x(g,params.t,params.t_min,params.t_max); % eval ODE

% calculate inner part
for n = 1:params.N
    for k = 1:params.K
        inner = inner + log(y(n,k) / x(n,2)) * log(y(n,k) / x(n,2)); 
    end
end 
% y's 
